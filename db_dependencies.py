...
class RoomWrapper:
    ...
    #UPDATE CANCEL ROOM
    def update_cancel_room(self, type_id, idlogin):
        todaydate = date.today().strftime("%Y-%m-%d")

        cursor = self.connection.cursor(pymysql.cursors.DictCursor)
        sql = 'UPDATE room SET status = "{}", last_update = "{}", last_update_by = "{}" WHERE id = {}'.format(2, todaydate, idlogin, type_id)
        cursor.execute(sql)

        return "Room {} updated!".format(type_id)
    
    # def update_booking_status(self, type_id, idlogin):
    #     cursor = self.connection.cursor(pymysql.cursors.DictCursor)
    #     sql = 'UPDATE booking SET status = "{}", id_employee = "{}" WHERE id = {}'.format(2, idlogin)
    #     cursor.execute(sql)
